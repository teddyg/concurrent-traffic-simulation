...
T MessageQueue<T>::receive()
{
    ...
    // The received object should then be returned by the receive function. 
  	...
  	// take the lock on the mutex then release it and wait for a notification
  	std::unique_lock<std::mutex> u_lock(_mutex);
  	_condition.wait(u_lock, [this] { return !_queue.empty(); });
  	
  	// move the value from the queue to the local variable
...
void TrafficLight::cycleThroughPhases()
{
  	...
  	while(true) 
    {
        ...
        {
          	// set the message with move semantics
